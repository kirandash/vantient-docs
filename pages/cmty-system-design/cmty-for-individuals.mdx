...
- Quests
  - Browse quests feed containing quests by the brand.
...
This can be achieved by using a hybrid framework like Next.js, which supports both SSR and CSR.
...
- Use the **Intersection Observer API** to monitor when the marker element is entering or exiting another element or intersecting by a specified amount.
...
- Feed Quests
  ...
  - `aria-labelledby="<id>"` where the HTML tag containing the feed owner name has that id attribute.
